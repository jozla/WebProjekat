...
namespace UserStatefull
{
    ...
    /// </summary>
    internal sealed class UserStatefull : StatefulService, IUserStatefullCommunication
    {
        ...
        public async Task Register(UserModel user)
        {
            ...
            {
                var enumerator = (await usersDict.CreateEnumerableAsync(tx)).GetAsyncEnumerator();

                while (await enumerator.MoveNextAsync(default))
                {
                    var usertoDelete = enumerator.Current.Value;
                    if (usertoDelete.Id == Guid.Empty)
                    {
                        await usersDict.TryRemoveAsync(tx, usertoDelete.Id);
                    }
                }
